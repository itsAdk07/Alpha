...
void parse(char* str)
{
    ...
    while (right <= len && left <= right) {
        ...
        if (isDelimiter(str[right]) == true && left == right) {
            ...
            else if (isLogicalOperator(str[right]) == true)
                printf("'%c' IS AN Logical OPERATOR\n", str[right]);
...
int main()
{
    ...
    scanf("%99[^\n]", myArray); // Using %99 to prevent buffer overflow

    printf("You entered: %s\n", myArray);


parse(myArray); // calling the parse function
